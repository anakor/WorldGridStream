...
#include "WorldGridStreamHashHelpers.h"
...
uint32 FWorldGridStreamHashHelpers::CRCTablesSB8[8][256] = 
{
	{
...
FWorldGridStreamHashHelpers::MemCrc32Functor FWorldGridStreamHashHelpers::MemCrc32Func = &MemCrc32ArmHW;
#else
FWorldGridStreamHashHelpers::MemCrc32Functor FWorldGridStreamHashHelpers::MemCrc32Func = &MemCrc32SW;
#endif
...
struct CRCRuntimeHWSupportDetector
{
	...
	{
		FWorldGridStreamHashHelpers::MemCrc32Func = &MemCrc32SW;
#if PLATFORM_ANDROID_ARM64
		if (android_getCpuFeatures() & ANDROID_CPU_ARM64_FEATURE_CRC32)
		{
			FWorldGridStreamHashHelpers::MemCrc32Func = &MemCrc32ArmHW;
		}
#elif PLATFORM_LINUXARM64
...
#elif PLATFORM_IOS
		if (FPlatformMisc::CPUHasHwCrcSupport())
		{
			FWorldGridStreamHashHelpers::MemCrc32Func = &MemCrc32ArmHW;
		}
#else
		...
		{
			MemCrc32ArmHW(&SigAction, sizeof(SigAction));
			FWorldGridStreamHashHelpers::MemCrc32Func = &MemCrc32ArmHW;
		}
...
#endif
		if (FWorldGridStreamHashHelpers::MemCrc32Func == &MemCrc32ArmHW)
		{
...
static uint32 MemCrc32SW(const void* InData, int32 Length, uint32 CRC/*=0 */)
{
	...
	if (Length > InitBytes)
	{
		...
		for (; InitBytes; --InitBytes)
		{
			CRC = (CRC >> 8) ^ FWorldGridStreamHashHelpers::CRCTablesSB8[0][(CRC & 0xFF) ^ *Data++];
		}
		...
		for (uint32 Repeat = Length / 8; Repeat; --Repeat)
		{
			...
			uint32 V2 = *Data4++;
			CRC =
				FWorldGridStreamHashHelpers::CRCTablesSB8[7][ V1         & 0xFF] ^
				FWorldGridStreamHashHelpers::CRCTablesSB8[6][(V1 >> 8)   & 0xFF] ^
				FWorldGridStreamHashHelpers::CRCTablesSB8[5][(V1 >> 16)  & 0xFF] ^
				FWorldGridStreamHashHelpers::CRCTablesSB8[4][ V1 >> 24         ] ^
				FWorldGridStreamHashHelpers::CRCTablesSB8[3][ V2         & 0xFF] ^
				FWorldGridStreamHashHelpers::CRCTablesSB8[2][(V2 >> 8)   & 0xFF] ^
				FWorldGridStreamHashHelpers::CRCTablesSB8[1][(V2 >> 16)  & 0xFF] ^
				FWorldGridStreamHashHelpers::CRCTablesSB8[0][ V2 >> 24         ];
		}
